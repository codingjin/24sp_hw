...
def get_entropy(data_obj):
    ...
    p0 = float(data_obj.get_row_subset(attribute_name='label', attribute_value='F').__len__()) / data_size
    p1 = float(data_obj.get_row_subset(attribute_name='label', attribute_value='T').__len__()) / data_size
    if p0==0 or p1==0:
        return 0
...
def get_attribute_entropy(data_obj, attribute):
    ...
    for attribute_val in attribute_vals:
        ...
        p0 = float(sub_data_obj.get_row_subset(attribute_name='label', attribute_value='F').__len__()) / sub_data_size
        p1 = float(sub_data_obj.get_row_subset(attribute_name='label', attribute_value='T').__len__()) / sub_data_size
        if p0!=0 and p1!=0:
...
def get_best_attribute(data_obj, attributes, max_info_gain):
    ...
    for attribute in attributes:
        ...
        #print("attritebute=", attribute, " ig=", tmp_ig);
        if tmp_ig > maxig:
            maxig = tmp_ig
...
def walk(datarow, node):
    if node==None or datarow.any()==False:
...
attributes = ['variety', 'color', 'smell', 'time']
attributes_dict = {
    'variety' : ['A', 'K', 'H'],
    'color' : ['R', 'Y', 'G'],
    'smell' : ['N', 'S'],
