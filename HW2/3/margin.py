...
def cv_train(epoch_num, trainset, lr, u):
    if u <= 0:
...
def train(epoch_num, trainset, devset, lr, u):
    if u <= 0:
        print("Invalid u, u should > 0")
        exit()
...
def debug(epoch_num, lr, u, trainset, testset):
    w, b = w0, b0
...
LR_LIST, U_LIST = [1.0, 0.1, 0.01], [1.0, 0.1, 0.01]
...
for lr in LR_LIST:
    for u in U_LIST:
        ...
        for i in range(5):
            ...
            # training
            w, b = cv_train(CV_EPOCH, trainset, lr, u)
            # validation
        ...
        print("lr=%.2f and u=%.2f, cross-validation accuracy=%.2f%%, and standard deviation is %.2f" %(lr, u, validation_accuracy*100, stdvariance))

print("The best lr=%.2f and u=%.2f, the corresponding cross-validation accuracy = %.2f%%" % (best_lr, best_u, best_accuracy*100))
del dataset
lr = best_lr
u = best_u
...
best_w, best_b, accuracies = train(TRAIN_EPOCH, trainset, devset, lr, u)
...
train_acc, test_acc = debug(DEBUG_EPOCH, lr, u, trainset, testset)
