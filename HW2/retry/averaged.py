...
def train(epoch_num, trainset, devset, lr):
    ...
    aw, ab = 0.0, 0.0
    best_aw, best_ab, best_accuracy, accuracies = 0, 0, -0.01, []
    for epoch in range(epoch_num):
        np.random.shuffle(trainset)
        for datarow in trainset:
            y, x = datarow[0], datarow[1:]
        ...
        devset_accuracy = test(aw, ab, devset)
        accuracies.append(devset_accuracy)
    ...
    return (best_aw, best_ab, accuracies)
...
best_aw, best_ab, accuracies = train(TRAIN_EPOCH, trainset, devset, lr)

# Test
testset = pd.read_csv(DATA_DIR + 'diabetes.test.csv', header=0).to_numpy()
print("The test set accuracy is %.2f%%" % (100*test(best_aw, best_ab, testset)))
