...
def cv_train(epoch_num, trainset, u):
    if u <= 0:
        print("Invalid u, u should > 0")
        exit()
    w, b = w0, b0
    for epoch in range(epoch_num):
        ...
        for datarow in trainset:
            ...
            if y*(np.dot(x, w) + b) <= u:
                lr = ((u - y*(np.dot(x, w) + b))) / (np.dot(x, x) + 1)
                w += lr*y*x
                b += lr*y
    return (w, b)
...
def train(epoch_num, trainset, devset, u):
    if u <= 0:
        print("Invalid u, u should > 0")
        exit()
    ...
    for epoch in range(epoch_num):
        ...
        for datarow in trainset:
            ...
            if y*(np.dot(x, w) + b) <= u:
                lr = ((u - y*(np.dot(x, w) + b))) / (np.dot(x, x) + 1)
                w, b, update_num = w+lr*y*x, b+lr*y, update_num+1
...
def debug(epoch_num, u, trainset, testset):
    w, b = w0, b0
    ...
    for epoch in range(epoch_num):
        ...
        for datarow in trainset:
            ...
            if y*(np.dot(x, w) + b) <= u:
                lr = ((u - y*(np.dot(x, w) + b))) / (np.dot(x, x) + 1)
                w, b, train_mistake = w+lr*y*x, b+lr*y, train_mistake+1
...
U_LIST = [1.0, 0.1, 0.01]
best_lr, best_accuracy, best_u = 0, -0.1, 0
for u in U_LIST:
    ...
    for i in range(5):
        ...
        # training
        w, b = cv_train(CV_EPOCH, trainset, u)
        # validation
    ...
    print("u=%.2f, cross-validation accuracy=%.2f%%, and standard deviation is %.2f" %(u, validation_accuracy*100, stdvariance))

print("The best u=%.2f, the corresponding cross-validation accuracy = %.2f%%" % (best_u, best_accuracy*100))
del dataset
u = best_u
...
best_w, best_b, accuracies = train(TRAIN_EPOCH, trainset, devset, u)
...
train_acc, test_acc = debug(DEBUG_EPOCH, u, trainset, testset)
