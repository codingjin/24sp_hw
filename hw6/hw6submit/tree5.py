...
random.seed(43)
np.random.seed(43)
DATA_DIR, LIMIT_DEPTH, attributes_dict = 'data/', 5, {}
...
# Step 1: Generate 100 trees each with depth=5
nodes = []
...
# Step 3: Generate tree5_test.csv
df_test_data = pd.read_csv(DATA_DIR+'test.csv', dtype=int)
with open('tree5_test.csv', 'w', newline='') as file:
    writer = csv.writer(file)
    writer.writerow(header)
