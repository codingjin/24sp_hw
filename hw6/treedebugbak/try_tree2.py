...
with open(DATA_DIR + 'train.csv', 'r') as f:
    ...
    trainlist0 = list(reader)
    attributes = trainlist0[0][1:]
    trainlist = []
    trainlist.append(trainlist0[0])
    size = len(trainlist0)
    for i in range(size-1):
        temp = []
        for elem in trainlist0[i+1]:
            temp.append(round(float(elem)))
...
with open(DATA_DIR + 'test.csv', 'r') as f:
    reader = csv.reader(f, delimiter=',')
    testlist0 = list(reader)
    testlist = []
    testlist.append(testlist0[0])
    size = len(testlist0)
    for i in range(size-1):
        temp = []
        for elem in testlist0[i+1]:
            temp.append(round(float(elem)))
...
#exit()
SAMPLE_SIZE = int((len(trainlist)-1)*0.1)
#print(SAMPLE_SIZE)
...
class Node:
    ...
    def get_child(self, feature_value):
        if (self.children_num != 0):    
            #print("self.children")
            #print(self.children)
            return self.children[str(feature_value)]
        else:                           return None
...
"""
"""
with open(DATA_DIR + 'debug02.csv', 'r') as f:
    reader = csv.reader(f, delimiter=',')
    debuglist0 = list(reader)
    debuglist = []
    debuglist.append(debuglist0[0])
    size = len(debuglist0)
    for i in range(size-1):
        temp = []
        for elem in debuglist0[i+1]:
            temp.append(round(float(elem)))
        debuglist.append(temp)
    del debuglist0
"""
debug_df = pd.read_csv(DATA_DIR+'debug02.csv', dtype=int)
print("list")
print(list(debug_df))
print("values")
print(debug_df.values.tolist())

debuglist = []
debuglist.append(list(debug_df))
value_list = debug_df.values.tolist()
for row in value_list:
    debuglist.append(row)
print(debuglist)
exit()

debugdata_obj = Data(data=debuglist)
node = ID3(debugdata_obj, attributes, current_level=0)
...
print("On the training set, train_correct=%d, train_data_size=%d" % (train_correct, train_data_size))

#"""
df_test_data = pd.read_csv(DATA_DIR+'ftest.csv', dtype=int)
...
print("test_correct=%d test_data_size=%d" % (test_correct, test_data_size))
#"""
