...
TRAIN_SIZE, SAMPLE_SIZE, TREE_NUM = len(trainlist)-1, int((len(trainlist)-1)*0.2), 21
...
print("Accuracy on the training set is %.4f%%" % (100*float(train_data_size-num_mistake)/train_data_size))
...
for index, row in df_test_data.iterrows():
    ...
    common_label = get_common(temp_results)
    if int(row['label'])!=int(common_label):    num_mistake += 1
print("Accuracy on the test set is %.4f%%" % (100*float(test_data_size-num_mistake)/test_data_size))
