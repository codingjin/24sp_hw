...
class Data:
	...
	def get_attribute_possible_vals(self, attribute_name):
		...
		#return np.unique(self.raw_data[:, self.get_column_index(attribute_name)])
		return np.unique(self.raw_data[:, self.get_column_index(attribute_name)].astype(str))
	...
	def set_missing_feature(self, attribute_name, to_set_value):
		data = self.raw_data
		column_index = self.get_column_index(attribute_name)
		data[data[:, column_index]=='?', column_index] = to_set_value